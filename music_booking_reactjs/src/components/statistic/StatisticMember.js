...
export function StatisticalMember() {
  ...
  const totalItems = sortedChartData.length;
  ...
  const labels = chartData ? chartData.map((chart) => chart.memberName) : [];
    ...
    datasets: [
      {
        ...
        data: chartData ? chartData.map((chart) => chart.totalTickets) : [],
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8080/api/admin/statistics/member",
          {
            headers,
          }
        );
                    ...
                    {paginatedChartData.length === 0 ? (
                      ...
                      paginatedChartData.map((chart, index) => (
                        <tr key={index} className="text-center">
                          ...
                          <td>{chart.memberName}</td>
                          <td>{chart.totalTickets}</td>
                          <td>{chart.totalRevenue}</td>
                          <td>{chart.loyaltyPoints}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
